...
-(id)init
{
    self.following=NO;
    self.venues=nil;
    return [ self initWithNibName:@"VenueItemView" bundle:nil ];
}
...
-(void)viewDidAppear:(BOOL)animated
{
   ...
   for(NSMutableDictionary *adj in self.venues)
   {
       
       if([[NSString stringWithFormat:@"%@",self.venue.name] isEqualToString:[NSString stringWithFormat:@"%@",[adj valueForKey:@"dj"]]])
       {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ( section == 0 )
    {
        ...
        UIImage *img=
            [UIImage imageWithData:[NSData dataWithContentsOfURL:[NSURL URLWithString:self.venue.pic_path]]];
...
-(IBAction) showScheduleClicked:(id)sender
{
    ...
    [ app showDJSchedule:self:self.venue];
#endif
}
...
-(IBAction) followButtonClicked:(id)sender
{
    ...
    [ arr addObject:self.venue.name ];
    [ arrids addObject:self.venue.vid ];
...
-(void) got_dj:(NSDictionary *)data
{
    ...
    else
    {
        ...
        [ venue autorelease ];
        venue.name = [ dct objectForKey:@"name" ];
        venue.vid = [ dct objectForKey:@"id" ];
        venue.pic_path=[dct objectForKey:@"pic"];
        self.venue = venue;
        self.getv = nil;
...
-(void) got_followdjs:(NSDictionary *)data
{
    ...
    if (st>0 )
    {
        self.venues = [ data objectForKey:@"results" ];
