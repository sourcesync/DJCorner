...
-(id) init
{
    self = [ self initWithNibName:@"VenuesTabView" bundle:nil ];
    return self;
}
...
-(void)viewWillDisappear:(BOOL)animated
{
    ...
    if ( self.get_mode != VenuesGetDone )
    {
        self.get_mode = VenuesGetPaused;
        [ self.getter cancel];
    }
...
-(void)viewDidAppear:(BOOL)animated
{
    ...
    else
    {
        if (self.refresh)
        {
            ...
            self.get_mode = VenuesGetGetting;
            [ self.getter getNext ];
        ...
        {
            if ( self.getter && ( self.get_mode == VenuesGetPaused ) )
            {
                self.get_mode = VenuesGetGetting;
                [ self.getter getNext ];
            }
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
    ...
    {
        NSInteger count = [ self.getter.venues count ] ;
...
{    
    ...
    else if ( self.getter.venues == nil )
    {
        UITableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:
                                 @"noitem_cell"];
        if (cell == nil) 
    ...
    else if ( [ self.getter.venues count ] == 0 )
    {
        UITableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:
                                 @"noitem_cell"];
        if (cell == nil) 
...
#else
        if ( row == ([ self.getter.venues count] ) )
#endif
        {
            UITableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:
                                     @"getmore_cell"];
...
#else
            Venue *v = [ self.getter.venues objectAtIndex:row ];
#endif
            //  Set dj text...
            NSString *str = [ NSString stringWithFormat:@"%@",v.name];
            //cell.textLabel.text = str;
            cell.content.text=str;
            
            if (v.upcoming) cell.upcoming.text = v.upcoming;
...
-(void)loadImageForRow: (DjsCell *)tcell: (NSIndexPath *)path
{
    ...
    if ( [ tcell isKindOfClass: [ DjsCell class ] ] )
    {
        ...
        else if (! self.scrolling )
        {
            ...
            {
                //  Launch the getter...
                [ self.getter asyncGetPic:v.pic_path:idx ];
...
#else
    if((self.getter.venues==nil)||([self.getter.venues count]==0))   
    {
        return 44;
    }
    else
...
#pragma mark - venues getter delegate...

-(void) got_venues:(NSInteger)start :(NSInteger)end
{
    self.get_mode = VenuesGetPaused;
