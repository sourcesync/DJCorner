...
#
def get_dj_details( connection, djid ,language):

        # get djs collection...
        djs = _get_djs_col( connection )

	bid = bson.objectid.ObjectId( djid )
	obj = djs.find_one( { "_id":bid ,"language":language} )
	if not obj:
		return False
...
#
def get_djs_details( connection, searchrx, all, paging ,language):
	...
	if searchrx == None or searchrx.strip() == "":
		results = djs.find({"language":language})
	else:
		regex = ".*%s.*" % searchrx
		results = djs.find( {"name": re.compile(searchrx,re.IGNORECASE),"language":language } )
...
#
def get_similar_djs( connection, djid ,language):
