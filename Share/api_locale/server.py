...
class API(jsonrpc.JSONRPC):
        ...
        def jsonrpc_get_events(self,location,paging,city,all,language):
		print "INFO: api: get_events->", location, paging, "city->",city, all,language
		[ events, info ] = event.get_events_details( None, location, paging, city, all,language)
		dct = { "results":events, "status":1, "paging":info }
		#print "INFO: api: get_events: return dct->", dct
		return dct
        
	def jsonrpc_get_event(self,location,eoid,language):
		bid = bson.objectid.ObjectId(eoid)
		print "INFO: api: get_events->", location, eoid, bid,language
		evt = event.get_event_details( None, location, bid,language,True )
		if not evt:
	...
	def jsonrpc_get_djs(self, searchrx, all, paging,language):
		print "INFO: api: get_djs->", searchrx, all, paging, language
		status = dj.get_djs_details( None, searchrx, all, paging ,language)
		if not status:
			dct = {'status':0,'msg':'Problem with this operation.'}
			return dct	
		else:
	...
	def jsonrpc_get_dj(self, djid,language):
		print "INFO: api: get_dj->", djid
		status = dj.get_dj_details( None, djid,language )
		if not status:
			dct = {'status':0,'msg':'Problem with this operation.'}
			return dct	
		else:
	...
	def jsonrpc_get_similar_djs(self, djid,language):
		print "INFO: api: get_similar_djs->", djid,language
		status = dj.get_similar_djs( None,  djid ,language)
		if status == False:
			dct = {'status':0,'msg':'Problem with this operation.'}
			return dct	
		else:
