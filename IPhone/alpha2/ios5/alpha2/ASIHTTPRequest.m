...
- (void)checkRequestStatus
{
	...
	if ([self readStream]) {
		...
		if ([self postLength]) {
            ...
            //[self setTotalBytesSent:[NSMakeCollectable((NSNumber *)CFReadStreamCopyProperty((CFReadStreamRef)[self readStream], kCFStreamPropertyHTTPRequestBytesWrittenCount)) unsignedLongLongValue]]; //gw-analyze
...
- (void)readResponseHeaders
{
	...
	if ([self shouldAttemptPersistentConnection]) {
		...
		//NSString *httpVersion = 
            //NSMakeCollectable((NSString *)CFHTTPMessageCopyVersion(message)); //gw-analyze
...

{
   ...
   // NSError *underlyingError = 
     //   NSMakeCollectable((NSError *)CFReadStreamCopyError((CFReadStreamRef)[self readStream]) ); //gw-analyze
	...
	if (![self isPACFileRequest] && (![self proxyHost] && ![self proxyPort])) {
		...
		if ([self PACurl]) {
            ...
            //NSDictionary *proxySettings = 
                //NSMakeCollectable((NSDictionary *)CFNetworkCopySystemProxySettings()); //gw-analyze
#else
            ...
            //proxies = NSMakeCollectable((NSArray *)CFNetworkCopyProxiesForURL((CFURLRef)[self url], (CFDictionaryRef)proxySettings)); //gw-analyze
...
- (void)runPACScript:(NSString *)script
{
	if (script) {
        ...
        //NSArray *proxies = NSMakeCollectable((NSArray *)CFNetworkCopyProxiesForAutoConfigurationScript((CFStringRef)script,(CFURLRef)[self url], &err) ); //gw-analyze
...
+ (NSString *)mimeTypeForFileAtPath:(NSString *)path
{
    ...
    return 
        NSMakeCollectable([(NSString *)MIMEType autorelease]);
