...
- (void)checkRequestStatus
{
	...
	if ([self readStream]) {
		...
		if ([self postLength]) 
        {
            ...
            NSNumber *num = \
                (NSNumber *)CFReadStreamCopyProperty( \
                    (CFReadStreamRef)[self readStream], \
                    kCFStreamPropertyHTTPRequestBytesWrittenCount);
            unsigned long long ull = [ NSMakeCollectable( num ) unsignedLongLongValue ];
            [self setTotalBytesSent:ull ];
...
- (void)readResponseHeaders
{
	...
	if ([self shouldAttemptPersistentConnection]) {
		...
		NSString *httpVersion = 
            NSMakeCollectable((NSString *)CFHTTPMessageCopyVersion(message)); //ANA
...
{	
    ...
    //[self setTotalBytesSent:[NSMakeCollectable([(NSNumber *)CFReadStreamCopyProperty((CFReadStreamRef)[self readStream], kCFStreamPropertyHTTPRequestBytesWrittenCount) autorelease]) unsignedLongLongValue]];
    NSNumber *num = (NSNumber *)CFReadStreamCopyProperty((CFReadStreamRef)[self readStream], kCFStreamPropertyHTTPRequestBytesWrittenCount);
    unsigned long long ull = [NSMakeCollectable(num) unsignedLongLongValue];
    [self setTotalBytesSent: ull ];
...

{
	 //NSError *underlyingError = \
        NSMakeCollectable([(NSError *)CFReadStreamCopyError((CFReadStreamRef)[self readStream]) autorelease]);
    NSError *underlyingError = 
        NSMakeCollectable((NSError *)CFReadStreamCopyError((CFReadStreamRef)[self readStream]) ); //ANA
	...
	if (![self isPACFileRequest] && (![self proxyHost] && ![self proxyPort])) {
		...
		if ([self PACurl]) {
			...
			NSDictionary *proxySettings = \
                NSMakeCollectable( (NSDictionary *)CFNetworkCopySystemProxySettings() );
            //NSDictionary *proxySettings = 
                //NSMakeCollectable((NSDictionary *)CFNetworkCopySystemProxySettings()); //ANA
#else
			NSDictionary *proxySettings = NSMakeCollectable([(NSDictionary *)SCDynamicStoreCopyProxies(NULL) autorelease]);
#endif
            [ proxySettings autorelease ]; //ANA

			proxies = NSMakeCollectable( (NSArray *)CFNetworkCopyProxiesForURL((CFURLRef)[self url], (CFDictionaryRef)proxySettings));
            //proxies = NSMakeCollectable((NSArray *)CFNetworkCopyProxiesForURL((CFURLRef)[self url], (CFDictionaryRef)proxySettings)); //ANA
            [ proxies autorelease]; //ANA
...
- (void)runPACScript:(NSString *)script
{
	if (script) {
        ...
        NSArray *proxies = NSMakeCollectable((NSArray *)CFNetworkCopyProxiesForAutoConfigurationScript((CFStringRef)script,(CFURLRef)[self url], &err) ); //ANA
        [ proxies autorelease ]; //ANA
...
+ (NSString *)mimeTypeForFileAtPath:(NSString *)path
{
    ...
    return 
    [ NSMakeCollectable( (NSString *)MIMEType ) autorelease ];  //ANA
        //NSMakeCollectable((NSString *)MIMEType); //gw-analyze
}
